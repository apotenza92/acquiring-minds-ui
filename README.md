...
Use `--llm-provider codex_cli` or `AMKB_LLM_PROVIDER=codex_cli` to run extraction through the local `codex exec` subscription path. Use `openai_api` for the direct Responses API path.

`lessons:extract-episodes` reads transcript segments from SQLite, chunks each episode, and writes one structured extraction JSON per episode under `.corpus/acquiring-minds/extractions/episodes/`. It supports `--episode-id`, `--sample`, `--limit`, `--all`, `--force`, `--retry-failed`, `--dry-run`, `--episode-model`, `--episode-reasoning-effort`, `--llm-provider`, and `--max-chunk-chars`. The default episode model is `AMKB_EPISODE_MODEL` or `gpt-5.5`; the default reasoning effort is `AMKB_EPISODE_REASONING_EFFORT` or `low`; the default chunk size is `90000` characters. Use `--force` to regenerate existing episode extraction files with the current prompt/model/chunk settings.

`lessons:cluster` reads episode extraction JSON only, groups lesson candidates by ETA category, and writes cluster drafts under `.corpus/acquiring-minds/extractions/clusters/`. It supports `--sample`, `--limit`, `--all`, `--force`, `--retry-failed`, `--dry-run`, `--cluster-model`, `--cluster-reasoning-effort`, `--llm-provider`, and `--max-candidates-per-category`. The default cluster model is `AMKB_CLUSTER_MODEL` or `gpt-5.5`; the default reasoning effort is `AMKB_CLUSTER_REASONING_EFFORT` or `high`.

The shared model client defaults to `AMKB_LLM_PROVIDER` or `openai_api`, `OPENAI_MODEL` or `gpt-5.5`, and `OPENAI_REASONING_EFFORT` or `medium`. Valid reasoning efforts are `none`, `minimal`, `low`, `medium`, and `high`; the extraction scripts intentionally use lower effort for factual per-episode extraction and higher effort for cross-episode synthesis.
