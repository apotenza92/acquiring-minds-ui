...
The shared OpenAI client defaults to `OPENAI_MODEL` or `gpt-5.5`, and `OPENAI_REASONING_EFFORT` or `medium`. Valid reasoning efforts are `none`, `minimal`, `low`, `medium`, `high`, and `xhigh`; the extraction scripts intentionally use lower effort for factual per-episode extraction and higher effort for cross-episode synthesis.
